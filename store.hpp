...
class Header {
 public:
  Header(int32_t ts, int32_t key_size, int32_t value_size)
      : timestamp_(ts), key_sz_(key_size), value_sz_(value_size) {}
  ...
  int32_t KeySize() { return key_sz_; }
  int32_t ValueSize() { return value_sz_; }
...
class DiskStorage {
 public:
  ...
  std::string Get(std::string key) {
    ...
    f_.seekg(off);
    f_.read((char*)&hdr[0], kHeaderSize);
    header.Deserialize(hdr);

    auto sz = header.KeySize() + header.ValueSize();
    std::vector<uint8_t> bytes(sz);
    f_.seekg(off + kHeaderSize);
    f_.read((char*)&bytes[0], sz);

    std::string val(bytes.begin() + header.KeySize(), bytes.end());
    return val;
  }
 ...
 private:
  NaiveMapStorage<K, int32_t> index_;
